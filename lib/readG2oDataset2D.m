function [edges, readPoses, weights, P_xi_inv, P_delta_inv, At, A, A_full, A_2t, A_2] = readG2oDataset2D(input_file_g2o, doSortEdges)
  %
  %% INPUTS:
  ...
  while ischar(tline)
    ...
    if foundEdge==1
      ...
      P_delta_inv(edgeId,edgeId) = I33;
      indicesXi = 2*edgeId-1:2*edgeId;
      P_xi_inv(indicesXi,indicesXi) = [I11, I12; I12, I22];
    end
    
    foundSwitchableEdge = strfind(tline, 'EDGE_SE2_SWITCHABLE '); % note: we neglect lines "VERTEX_SWITCH" and "EDGE_SWITCH_PRIOR"
    %% EDGE_SE2_SWITCHABLE lines
    if isempty(foundSwitchableEdge)==0
      edgeId = edgeId+1;
      [name, id1, id2, eId, dx, dy, dth, I11, I12, I13, I22, I23, I33] = strread(tline, '%s %d %d %d %f %f %f %f %f %f %f %f %f');
      if eId~=edgeId
        error('loadG2oDataset: incorrect switchable edge ids in g2o file')
      end
      edges(edgeId,:) = [id1+1, id2+1, dx, dy, dth, I11, I12, I13, I22, I23, I33];
      weights(edgeId) = 1/I33; % angular measurement variance
      P_delta_inv(edgeId,edgeId) = I33;
  ...
  %% Check validity of the edges we read
  if unorderedFlag==1 && doSortEdges == 1
    disp('readG2oDataset2D: incorrect edge ids in g2o file, we are going to sort them')
    [edges, weights, P_xi_inv, P_delta_inv] = sortEdges(edges, weights, P_xi_inv, P_delta_inv);
  end
  
  fclose(fid_g2o);
  
  %% Compute other structures describing the graph
  % [A, At, At_full] = reducedIncidenceMatrix(edges);
  % A_full = At_full';
  % A_2t = kron(At, eye(2));
  % A_2 = kron(A, eye(2));
end
